...
import (
	...
	"github.com/sicko7947/gorkflow/example/sequential"
...
type WorkflowStatus struct {
	*gorkflow.WorkflowRun
	...
	Output         *sequential.FormatOutput `json:"output,omitempty"`
}
...
func initializeApp() {
	...
	workflow, err = sequential.NewSimpleMathWorkflow()
	if err != nil {
...
func handleStartWorkflow(c fiber.Ctx) error {
	var input sequential.WorkflowInput
...
func handleGetStatus(c fiber.Ctx) error {
	...
	if run.Status == gorkflow.RunStatusCompleted && len(run.Output) > 0 {
		var output sequential.FormatOutput
